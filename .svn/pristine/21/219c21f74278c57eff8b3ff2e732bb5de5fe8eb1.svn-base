...
namespace TwoPole.Chameleon3.Business.JingTang.Modules
{
    public class ExamModule : ModuleBase
    {
        ...
        protected virtual async Task StartExamAsync(ExamContext context)
        {
                ...
                || context.ExamTimeMode == ExamTimeMode.Night && Settings.CheckLightingSimulation && Settings.SimulationsLightOnNight)
            {
                ...
                //如果没有插上安全带系统会等待用户插上安全带
                //等待关闭发动机

                if (context.ExamTimeMode == ExamTimeMode.Day)
                {
                    await Task.Run(() =>
                    {
                        while (!(SignalSet.Current != null && SignalSet.Current.Sensor.SafetyBelt))
                        {
                            Thread.Sleep(100);
                        }
                    });
