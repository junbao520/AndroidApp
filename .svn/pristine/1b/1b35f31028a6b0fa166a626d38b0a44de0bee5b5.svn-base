...
namespace TwoPole.Chameleon3.Business.Areas.HaiNan.HaiKou.ExamItems
{
    ...
    /// 16，转向灯超时检测
    /// 17, 三模以及车头语音
    /// 特殊：从三亚移植，有车尾语音（绕车2），20170714
    /// </summary>
    public class CommonExamItem_HaiHe : ExamItemBase
    {
        #region 私有变量
        ...
        //    messenger.Register<SafetyBeltMessage>(this, OnSafetyBeltMessage);
        ...
        /// <summary>
        /// 是否已经检测过起步3分钟超时（上车准备到起步不能超过3分钟）
        /// </summary>
        private bool IsCheckVehicleTimeOut = false;
        /// <summary>
        /// 检测3分钟超时（从上车准备到起步3分钟）
        /// </summary>
        /// <param name="signalInfo"></param>
        private void ChecrnkTimeOut(CarSignalInfo signalInfo)
        {
            if (IsCheckVehicleTimeOut)
                return;
            if (Context.ExamMode != ExamMode.Examming)
                return;
            if ((DateTime.Now - StartTime).TotalSeconds > 180)
            {
                IsCheckVehicleTimeOut = true;
                BreakRule(DeductionRuleCodes.RC30103, DeductionRuleCodes.SRC3010301);
            }
            else
            {
                //5码，则认为起步成功
                if (signalInfo.SpeedInKmh > 5)
                    IsCheckVehicleTimeOut = true;
        ...
        {
            Logger.Info(TouchInnerMirror.ToString());
            Logger.Info(signalInfo.Sensor.ToString());
            if (!TouchInnerMirror)
            {
                if (signalInfo.Sensor.InnerMirror)
                {
                    Logger.Info("InnerMirror");
                    TouchInnerMirror = true;
                    Speaker.PlayAudioAsync("后视",SpeechPriority.Normal);
                }
            }
            ...
            if (!TouchSeats)
            {
                if (signalInfo.Sensor.Seats)
                {
                    Logger.Info("Seats");
                    TouchSeats = true;
                    Speaker.PlayAudioAsync("座椅", SpeechPriority.Normal);
                }
            }
            ...
            if (!ArrivedHeadstock)
            {
                ...
                {
                    Logger.Info("ArrivedHeadstock");
                    ArrivedHeadstock = true;
                    Speaker.PlayAudioAsync("绕车一", SpeechPriority.Normal);
                }
            }
            if (!ArrivedTailstock)
            {
                if (signalInfo.Sensor.ArrivedTailstock)
                {
                    ...
                    Speaker.PlayAudioAsync("绕车二", SpeechPriority.Normal);
                }
            }
            if (!TouchExteriorMirror)
            {
                if (signalInfo.Sensor.ExteriorMirror)
                {
                    ...
                    Logger.Info("ExteriorMirror");
                    TouchExteriorMirror = true;


                    Speaker.PlayAudioAsync("观望",SpeechPriority.Normal);
        ...
        //    }
        //}
        private void OnSafetyBeltMessage(SafetyBeltMessage message)
        {
           // BreakRule(DeductionRuleCodes.RC30135);
            CheckRule(true, DeductionRuleCodes.RC30135);
        }
        ...
        protected override void ExecuteCore(CarSignalInfo signalInfo)
        {
            ...
            {
                if (!signalInfo.Sensor.SafetyBelt&& signalInfo.CarState == CarState.Moving)
                {
