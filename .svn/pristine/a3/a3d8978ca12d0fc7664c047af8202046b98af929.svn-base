...
namespace TwoPole.Chameleon3.Business.ExamItems
{
    ...
    public class ModifiedGear : ExamItemBase
    {
        ...
        //减档语音
        protected List<KeyValuePair<Gear, string>> lstKeyValueSubGearVoice;
        //加档语音
        protected List<KeyValuePair<Gear, string>> lstKeyValueAddGearVoice;

        protected string[] GearFlowVoice;
        ...
        public override void Init(NameValueCollection settings)
        {
           ...
           // Logger.Error("ModifiedGearFlowVoice" + Settings.ModifiedGearFlowVoice);
            if (string.IsNullOrEmpty(Settings.ModifiedGearFlowVoice))
            {

                GearFlowVoice = DefaultGlobalSettings.DefaultModifiedGearFlowVoice.Split('-');
               // Logger.Error("ModifiedGearFlowVoiceLength:" + GearFlowVoice+";" + GearFlowVoice.Length.ToString());
            }
            else
            {
                GearFlowVoice = Settings.ModifiedGearFlowVoice.Split('-');
               // Logger.Error("else ModifiedGearFlowVoiceLength:" + GearFlowVoice + ";" + GearFlowVoice.Length.ToString());
            }
            ...
            //1-2;2-3;3-4;4-5;
            //5-4;4-3;3-2;2-1;
            lstKeyValueAddGearVoice = new List<KeyValuePair<Gear, string>>() {
                new KeyValuePair<Gear, string>(Gear.One,GearFlowVoice[0]),
                new KeyValuePair<Gear, string>(Gear.Two,GearFlowVoice[1]),
                new KeyValuePair<Gear, string>(Gear.Three,GearFlowVoice[2]),
                new KeyValuePair<Gear, string>(Gear.Four,GearFlowVoice[3])
            };
            lstKeyValueSubGearVoice = new List<KeyValuePair<Gear, string>>() {
                new KeyValuePair<Gear, string>(Gear.Five,GearFlowVoice[4]),
                new KeyValuePair<Gear, string>(Gear.Four,GearFlowVoice[5]),
                new KeyValuePair<Gear, string>(Gear.Three,GearFlowVoice[6]),
                new KeyValuePair<Gear, string>(Gear.Two,GearFlowVoice[7])
            };
            //List<Gear> lstTempGear = new List<Gear>();
            ////档位流程测试2-3-4-3-2
        ...
        protected override void ExecuteCore(CarSignalInfo signalInfo)
        {
            ...
            //按操作结束项目
            if (Settings.ChkModifiedGearEndFlag && IsSuccess)
            {
                StopCore();
            }
        ...
        protected Gear GetNextGear()
        {
            ...
            try
            {
                ...
                //当流程为最后一个时，返回空挡，20180802
                if (LstGearFlow.Count > 0 && LstGearFlow[GearFlowIndex].Count > 0)
                    gear = LstGearFlow[GearFlowIndex][0];
                return gear;
            }
            catch (Exception ex)
            {
        ...
        protected void PlayActionVoice(Gear NowGear, Gear NextGear)
        {
            ...
            {
                VocieText= lstKeyValueSubGearVoice.Where(s => s.Key == NowGear).FirstOrDefault().Value;
               // VocieText = "请减到" + NextGearNumber.ToString() + "档";
            }
            else
            {
                VocieText = lstKeyValueAddGearVoice.Where(s => s.Key == NowGear).FirstOrDefault().Value;
